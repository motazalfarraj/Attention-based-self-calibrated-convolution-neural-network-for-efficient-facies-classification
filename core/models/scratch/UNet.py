...
class DilatedUNet(nn.Module):
    ...
    def __init__(self, in_channels=1, n_classes=6, depth=4,
                 first_channels=32, padding=1,
                 bottleneck_depth=5, bottleneck_type='parallel',
        ...
        for i in range(bottleneck_depth):
            bneck_in = prev_channels if i == 0 else prev_channels * 1
    ...
    def forward(self, x):
        x_size = x.size()
        pad_h = int(np.ceil(x_size[-2] / 2**(self.depth+1)) * 2**(self.depth+1) - x_size[-2])
        pad_w = int(np.ceil(x_size[-1] / 2**(self.depth+1)) * 2**(self.depth+1) - x_size[-1])
