...
class BasicDcBlock(nn.Module):
    expansion = 1
    def __init__(self, in_channels, channels,
                 stride=1, groups=1, width_per_group=64, sd=0.0,
                 **block_kwargs):
        super(BasicDcBlock, self).__init__()
        ...
        self.conv1 = layers.dconv3x3(in_channels, width, stride=stride)
        self.conv2 = nn.Sequential(
            layers.bn(width),
            layers.relu(),
            layers.dconv3x3(width, channels * self.expansion),
        )
