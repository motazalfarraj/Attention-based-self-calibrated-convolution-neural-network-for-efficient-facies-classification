...
import core.models.ViT.models.resnet_dnn_block as resnet_dnn
import core.models.ViT.models.resnet_mcdo_block as resnet_mcdo
import core.models.ViT.models.preresnet_dnn_block as preresnet_dnn
import core.models.ViT.models.preresnet_mcdo_block as preresnet_mcdo
import core.models.ViT.models.classifier_block as classifier

from functools import partial
from itertools import cycle
from einops import rearrange
from core.models.ViT.models.layers import conv1x1, DropPath
from core.models.ViT.models.attentions import Attention2d
...
class AlterNet(nn.Module):
    ...
    def __init__(self, block1, block2, *,
        ...
        self.layer0 = stem(1, 64)
        self.layer1 = self._make_layer(block1, block2, 64, 64,
                                       num_blocks[0], num_blocks2[0], stride=1, heads=heads[0], sds=sds[0],
                                       **block_kwargs)
        self.layer2 = self._make_layer(block1, block2, 64 * block2.expansion, 128,
                                       num_blocks[1], num_blocks2[1], stride=2, heads=heads[1], sds=sds[1],
                                       **block_kwargs)
        self.layer3 = self._make_layer(block1, block2, 128 * block2.expansion, 256,
                                       num_blocks[2], num_blocks2[2], stride=2, heads=heads[2], sds=sds[2],
                                       **block_kwargs)
        self.layer4 = self._make_layer(block1, block2, 256 * block2.expansion, 512,
                                       num_blocks[3], num_blocks2[3], stride=2, heads=heads[3], sds=sds[3],
                                       **block_kwargs)

        # self.decode1 =

    @staticmethod
    def _make_layer(block1, block2, in_channels, out_channels, num_block1, num_block2, stride, heads, sds,
                    **block_kwargs):
...
#     def forward(self, x):
#         x_size = x.size()
#         pad_h = int(np.ceil(x_size[-2] / 7) * (7) - x_size[-2])
#         pad_w = int(np.ceil(x_size[-1] / 7) * (7) - x_size[-1])
...
# # Model
#
# class MyAlterNet(nn.Module):
#
#     def __init__(self, block1, block2, *,
#                  num_blocks, num_blocks2, heads,
#                  sd=0.0, num_classes=6, stem=StemB, name="cavit", **block_kwargs):
#         super().__init__()
#         self.name = name
#         idxs = [[j for j in range(sum(num_blocks[:i]), sum(num_blocks[:i + 1]))] for i in range(len(num_blocks))]
#         sds = [[sd * j / (sum(num_blocks) - 1) for j in js] for js in idxs]
...
#
#         self.conv1 = self._make_layer(block1, block2, 1, 32,
#                                        num_blocks[0], num_blocks2[0], stride=1, heads=heads[0], sds=sds[0],
#                                        **block_kwargs)
#
#         self.conv2 = self._make_layer(block1, block2, 32*block2.expansion, 64,
#                                        num_blocks[1], num_blocks2[1], stride=1, heads=heads[1], sds=sds[1],
#                                        **block_kwargs)
#
#         self.conv3 = self._make_layer(block1, block2, 64*block2.expansion, 64,
#                                        num_blocks[2], num_blocks2[2], stride=1, heads=heads[2], sds=sds[2],
#                                        **block_kwargs)
#
#         self.conv4 = self._make_layer(block1, block2, 64*block2.expansion, 128,
#                                        num_blocks[3], num_blocks2[3], stride=1, heads=heads[3], sds=sds[3],
#                                        **block_kwargs)
#
#         self.conv5 = self._make_layer(block1, block2, 128*block2.expansion, 128,
#                                        num_blocks[4], num_blocks2[4], stride=1, heads=heads[4], sds=sds[4],
#                                        **block_kwargs)
#
#         self.bottleneck = preresnet_dnn.Bottleneck(128*block2.expansion,128)
#
#         self.dconv5 = self._make_layer(block1, block2, 128 * block2.expansion, 128,
#                                        num_blocks[4], num_blocks2[4], stride=1, heads=heads[4], sds=sds[4],
#                                        **block_kwargs)
#
#         self.dconv4 = self._make_layer(block1, block2, 128*block2.expansion, 64,
#                                        num_blocks[3], num_blocks2[3], stride=1, heads=heads[3], sds=sds[3],
#                                        **block_kwargs)
#
#         self.dconv3 = self._make_layer(block1, block2, 64*block2.expansion, 64,
#                                        num_blocks[2], num_blocks2[2], stride=1, heads=heads[2], sds=sds[2],
#                                        **block_kwargs)
#
#         self.dconv2 = self._make_layer(block1, block2, 64*block2.expansion, 32,
#                                        num_blocks[1], num_blocks2[1], stride=1, heads=heads[1], sds=sds[1],
#                                        **block_kwargs)
#
#         self.dconv1 = self._make_layer(block1, block2, 32*block2.expansion, 32,
#                                        num_blocks[0], num_blocks2[0], stride=1, heads=heads[0], sds=sds[0],
#                                        **block_kwargs)
#
...
#
#         x1 = self.conv1(x)
#         x1_pool, index1 = self.pool(x1)
#
#         x2 = self.conv2(x1_pool)
#         x2_pool, index2 = self.pool(x2)
#
#         x3 = self.conv3(x2_pool)
#         x3_pool, index3 = self.pool(x3)
#
#         x4 = self.conv4(x3_pool)
#         x4_pool, index4 = self.pool(x4)
#
#         x5 = self.conv5(x4_pool)
#         x5_pool, index5 = self.pool(x5)
#
#         bottleneck = self.bottleneck(x5_pool)
#
#         y5_unpool = self.unpool(bottleneck, index5,x5.size())
#         y5 = self.dconv5(y5_unpool) + x4_pool
#
#         y4_unpool = self.unpool(y5, index4,x4.size())
#         y4 = self.dconv4(y4_unpool) + x3_pool
#
#         y3_unpool = self.unpool(y4, index3,x3.size())
#         y3 = self.dconv3(y3_unpool) + x2_pool
#
#         y2_unpool = self.unpool(y3, index2,x2.size())
#         y2 = self.dconv2(y2_unpool)
#
#         y1_unpool = self.unpool(y2, index1,x1.size())
#         y1 = self.dconv1(y1_unpool)
#
#         out = self.classify(y1)
#
#
#         return out
#
#
# def mydnn_18(num_classes=6, stem=True, name="alternet_18", **block_kwargs):
#     return MyAlterNet(preresnet_dnn.BasicBlock, AttentionBasicBlockB, stem=partial(StemA, pool=False),
#                     num_blocks=(2,2,2,2,2), num_blocks2=(1,1,1,1,1), heads=(3,6,12,24,48),
#                     num_classes=num_classes, name=name, **block_kwargs)
#
#
# #
...
# #
# # def dnn_18(num_classes=1000, stem=True, name="alternet_18", **block_kwargs):
# #     return AlterNet(preresnet_dnn.BasicBlock, AttentionBlockB, stem=partial(StemB, pool=stem),
# #                     num_blocks=(2, 2, 2, 2), num_blocks2=(0, 1, 1, 1), heads=(3, 6, 12, 24),
# #                     num_classes=num_classes, name=name, **block_kwargs)
# #
# #
