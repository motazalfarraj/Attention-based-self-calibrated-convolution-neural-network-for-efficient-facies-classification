...
class RandomZoom(object):
    def __call__(self, img, mask):
        assert img.size == mask.size
        if random.random()>0.5:
            scale = 2*random.random() + 1

            img_size = img.size
            x1 = random.randint(0, int(img.size[0] * (scale - 1)))
            y1 = random.randint(0, int(img.size[1] * (scale - 1)))
            img, mask = img.resize((int(img.size[0] * scale), int(img.size[1] * scale)), Image.BILINEAR), \
                        mask.resize((int(img.size[0] * scale), int(img.size[1] * scale)), Image.NEAREST)
            img = img.crop((x1, y1, x1 + img_size[0], y1 + img_size[1]))
            mask = mask.crop((x1, y1, x1 + img_size[0], y1 + img_size[1]))
